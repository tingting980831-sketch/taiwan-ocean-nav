...
if lons is not None:
    ...
    for zone in NO_GO_ZONES:
        poly = np.array(zone)
        ax.fill(poly[:,1], poly[:,0], color='red', alpha=0.4, label="禁航區")

    # 離岸風場 (半透明黃)
    for zone in OFFSHORE_WIND:
        poly = np.array(zone)
        ax.fill(poly[:,1], poly[:,0], color='yellow', alpha=0.4, label="離岸風場")

    # 航線
    if path:
        path_lons = [lons[p[1]] for p in path]
        path_lats = [lats[p[0]] for p in path]
        ax.plot(path_lons, path_lats, color='green', linewidth=2, label="航線")
