...
if st.sidebar.button("🚀 啟動即時導航分析"):
    try:
        ...
        if np.isnan(u_val):
            ...
            # 調整 Colorbar 避免擠壓地圖空間
            plt.colorbar(cf, label='Current Speed (m/s)', shrink=0.7, pad=0.05)
            ...
            st.pyplot(fig)
            st.success(f"HELIOS 系統運作中：已強制 1:1 比例顯示。")
