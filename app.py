...
# --- 1. 系統初始化 ---
st.set_page_config(page_title="HELIOS V27 最終鐵律版", layout="wide")
...
ocean_data, data_clock, stream_status = fetch_data_v27()

# --- 2. 鐵律：陸地判別 (含5公里緩衝) ---
def check_land_lock(lat, lon):
    # 台灣本島嚴格矩形遮罩 (含 5km 緩衝區)
    return (21.9 <= lat <= 25.4) and (120.1 <= lon <= 122.0)

# --- 3. 儀表板 (還原所有指標 + 建議航向) ---
st.title("🛰️ HELIOS V27 (硬性陸地攔截版)")
...
if st.session_state.real_p and st.session_state.step_idx < len(st.session_state.real_p)-1:
    p1 = st.session_state.real_p[st.session_state.step_idx]
    p2 = st.session_state.real_p[st.session_state.step_idx+1]
    ...
    brg_val = f"{(np.degrees(np.arctan2(y,x))+360)%360:.1f}°"
c3.metric("🧭 建議航向", brg_val)
...
m2.info("📡 GNSS: 訊號鎖定")
m3.write(f"📍 當前坐標: {st.session_state.ship_lat:.3f}N, {st.session_state.ship_lon:.3f}E")
st.markdown("---")
...
with st.sidebar:
    ...
    # 檢查是否觸發陸地限制
    start_on_land = check_land_lock(slat, slon)
    end_on_land = check_land_lock(elat, elon)

    if start_on_land or end_on_land:
        ...
        if st.button("🚀 啟動智能航路", use_container_width=True):
            ...
            path_pts = [[slat, slon]]
            # 跨越東西岸判斷
            if (slon < 121.1 and elon > 121.1) or (slon > 121.1 and elon < 121.1):
                if (slat + elat) / 2 > 23.8: # 繞北
                    path_pts.extend([[26.3, 121.5], [25.4, 122.7]])
                else: # 繞南
                    path_pts.extend([[21.8, 120.1], [20.7, 120.8], [21.5, 121.8]])
            path_pts.append([elat, elon])
            
            # 轉化為 100 個細分點 (純直線段)
            final_path = []
            for i in range(len(path_pts)-1):
                seg = np.linspace(path_pts[i], path_pts[i+1], 30)
                final_path.extend(seg.tolist())
            
            st.session_state.real_p = final_path
...
if st.button("🚢 執行下一階段航行", use_container_width=True):
    if st.session_state.real_p and st.session_state.step_idx < len(st.session_state.real_p)-1:
        ...
        curr_p = st.session_state.real_p[st.session_state.step_idx]
        st.session_state.ship_lat, st.session_state.ship_lon = curr_p[0], curr_p[1]
        st.rerun()
