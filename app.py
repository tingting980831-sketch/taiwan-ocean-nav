...
def fetch_ocean():
    try:
        ...
        # 將時間轉成可讀格式
        try:
            ocean_time=pd.to_datetime(ds.time.values[-1]).strftime("%Y-%m-%d %H:%M")
        except:
            base_time=datetime(2000,1,1)
            ocean_time=(base_time+timedelta(hours=float(ds.time.values[-1]))).strftime("%Y-%m-%d %H:%M")

        return sub.lon.values, sub.lat.values, sub.water_u.values, sub.water_v.values, ocean_time, "ONLINE"
    except:
...
BUFFER_DEG = 0.045
def is_land(lat,lon):
    taiwan = ((21.8-BUFFER_DEG <= lat <= 25.4+BUFFER_DEG) and
...
with st.sidebar:
    ...
    if st.button("📍 定位目前位置"):
        ...
        st.session_state.ship_lon = 122.2
...
def route_stats():
    ...
    for i in range(len(st.session_state.real_p)-1):
        total_dist+=haversine(st.session_state.real_p[i],st.session_state.real_p[i+1])
    remaining=st.session_state.real_p[st.session_state.step_idx:]
    if len(remaining)>=2:
        next_wp=remaining[1]
        cur_wp=remaining[0]
        dx=next_wp[1]-cur_wp[1]
        dy=next_wp[0]-cur_wp[0]
        heading=np.degrees(np.arctan2(dx,dy))%360
    else:
        heading="---"
    eta_hours=total_dist/speed_now if speed_now>0 else "---"
    return f"{total_dist:.1f} km",f"{eta_hours:.1f} hr",f"{heading}"

distance,eta,heading = route_stats()
...
r1[4].metric("🛣️ 剩餘航程",distance)
r1[5].metric("🕑 預計抵達",eta)
st.markdown("---")
...
ax.pcolormesh(lons,lats,speed_map,cmap="YlGn",alpha=0.7)
