...
def get_nav_data(u, v, clat, clon, dlat, dlon):
    ...
    vs_ms = 15.0 * 0.514 # 基礎船速 15 節轉公尺/秒
    
    # 計算 SOG (對地速度)：推力 + 海流對應方向的分量
...
if btn_analyze or btn_move:
    with st.spinner('📡 正在透過衛星下載即時海流圖...'):
        try:
            ...
            ax.quiver(c_lon, c_lat, hu, hv, color='#FF00FF', scale=4, width=0.015, label='AI Suggested (Pink)')
            ...
            st.pyplot(fig)
            st.success("✅ 數據傳輸完成：當前為 LEO 900km 模式，已優化能源路徑。")

        except Exception as e:
            st.error(f"衛星系統連線超時：{e}")
