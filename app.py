...
if lat is not None:
    ...
    # 計算 5km 對應的格點數 (每度約 111km)
    grid_res = lat[1] - lat[0]
    buffer_cells = int(5 / (111 * grid_res))
    ...
    # ===============================
    path = None
    dist_km, fuel_bonus, eta, brg_val = 0.0, 0.0, 0.0, "---"
    ...
    if run_btn:
        ...
        def flow_cost(i, j, ni, nj):
            ...
            return norm * (1 - 0.7 * assist) # 順流則成本降低 70%
    ...
    r1[3].metric("🌊 流場狀態", "正常運作" if fuel_bonus > 0 else "待機中")
    ...
    # 紅色 5km 緩衝界線 (極淡顯示)
    ax.contourf(lon, lat, forbidden, levels=[0.5, 1], colors=['red'], alpha=0.05, zorder=1)
    ...
    st.error("無法載入 HYCOM 資料，請確認網路連線。")
