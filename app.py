...
# 波高 surface
fig3d.add_trace(go.Surface(x=lon_grid,y=lat_grid,z=wave_surface,colorscale="Viridis",opacity=0.8))
# 海流箭頭
fig3d.add_trace(go.Cone(
    x=lon_grid.flatten(),y=lat_grid.flatten(),z=wave_surface.flatten()+0.2,
    u=u.flatten(),v=v.flatten(),w=np.zeros_like(u.flatten()),sizeref=2,showscale=False
))
# 風速箭頭
fig3d.add_trace(go.Cone(
    x=lon_grid.flatten(),y=lat_grid.flatten(),z=wave_surface.flatten()+0.4,
    u=np.full_like(u.flatten(),wind_u),v=np.full_like(v.flatten(),wind_v),w=np.zeros_like(u.flatten()),
    sizeref=4,colorscale="Reds"
))
# 航線
if path:
    plon=[lons[p[1]] for p in path]
...
fig3d.update_layout(height=700,scene=dict(xaxis_title="經度",yaxis_title="緯度",zaxis_title="高度"))
