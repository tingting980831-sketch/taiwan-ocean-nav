...
st.set_page_config(layout="wide", page_title="HELIOS V6 3D 模型導航")
...
def get_marine(lat, lon):
    ...
    wave = float(df_wave['wave_height'].iloc[0])

    # 風速
    wind_url = "https://api.open-meteo.com/v1/forecast"
    wind_params = {"latitude":lat,"longitude":lon,"hourly":["wind_speed_10m","wind_direction_10m"],"timezone":"Asia/Taipei","wind_speed_unit":"m/s","forecast_days":1}
    wind_res = requests.get(wind_url, params=wind_params).json()
    df_wind = pd.DataFrame(wind_res['hourly'])
    wind_speed = np.array(df_wind['wind_speed_10m'].iloc[0])
    wind_dir = np.array(df_wind['wind_direction_10m'].iloc[0])
    ...
    while pq:
        ...
        for d in dirs:
            ...
            if 0<=ni<rows and 0<=nj<cols and not land_mask[ni,nj]:
                ...
                # 加上波高與風速
                step += wave_factor*50 + wind_factor*20
                if safety[ni,nj]<4:
...
if lons is not None:
    ...
    wind_factor = np.sqrt(wind_u**2 + wind_v**2)

    path=astar(start,goal,u,v,land_mask,safety,ship_speed,wave_factor=wave,wind_factor=wind_factor)
...
# ----------------------------
    ...
    # 將流速 + 風速 + 波高加權
    total_factor = np.sqrt(u**2 + v**2) + wind_factor + wave
    im=ax.pcolormesh(lons,lats,total_factor,cmap=cmap,shading='auto',alpha=0.8)
    plt.colorbar(im,ax=ax,label="綜合海象因子",pad=0.05)
    ...
    plt.title("HELIOS V6 Navigation (2D 綜合底圖)")
    st.pyplot(fig2d)
...
# ----------------------------
    ...
    wave_grid = np.full_like(lon_grid, wave)
    wind_speed_grid = np.full_like(lon_grid, wind_factor)

    fig3d = go.Figure()

    # 波高在最下層
    fig3d.add_trace(go.Surface(
        x=lon_grid, y=lat_grid, z=wave_grid,
        colorscale='Blues', opacity=0.9, name="波高", showscale=True
    ))

    # 流速在中間
    fig3d.add_trace(go.Surface(
        x=lon_grid, y=lat_grid, z=flow_speed + wave_grid*0.2,
        colorscale='Viridis', opacity=0.6, name="流速", showscale=True
    ))
    ...
    fig3d.add_trace(go.Cone(
        ...
        z=flow_speed[::skip,::skip].flatten() + wave_grid[::skip,::skip].flatten() + 0.5,
        u=wind_u*np.ones_like(lon_grid[::skip,::skip].flatten()),
        v=wind_v*np.ones_like(lat_grid[::skip,::skip].flatten()),
    ...
    if path:
        ...
        fig3d.add_trace(go.Scatter3d(
            x=path_lons,
            y=path_lats,
            z=np.full(len(path_lons),flow_speed.max()+wave+1),
            mode='lines',
