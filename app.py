...
def load_hycom_data():
    ...
    try:
        ...
        # 🛡️ 模擬禁航區 (手動定義台灣西岸風場區域)
        # 格式: [lon_min, lon_max, lat_min, lat_max]
        no_go_zones = [
            [119.8, 120.2, 23.8, 24.5], # 苗栗/彰化外海風場群
            [119.5, 120.0, 23.0, 23.5], # 雲林/嘉義外海風場
        ]
        
        for zone in no_go_zones:
            lon_idx = np.where((lons >= zone[0]) & (lons <= zone[1]))[0]
            lat_idx = np.where((lats >= zone[2]) & (lats <= zone[3]))[0]
        ...
        return lons, lats, u_val, v_val, land_mask, latest_time, no_go_zones
    except Exception as e:
        st.error(f"📡 數據讀取失敗: {e}")
        return None, None, None, None, None, None, None

lons, lats, u, v, land_mask, obs_time, no_go_zones = load_hycom_data()
...
if lons is not None:
    with st.sidebar:
        st.header("📍 航點座標輸入")
        s_lon = st.number_input("起點經度", value=119.50, format="%.2f") # 調整預設起點避開風場
    ...
    st.caption(f"📅 數據時間: {obs_time.strftime('%Y-%m-%d %H:%M')} | 💡 系統已自動避開台灣西岸離岸風場")
    st.divider()
    ...
    # 🛑 視覺化禁航區 (風場)
    for zone in no_go_zones:
        rect = plt.Rectangle((zone[0], zone[2]), zone[1]-zone[0], zone[3]-zone[2],
                             facecolor='orange', alpha=0.4, hatch='//', label='Wind Farm', zorder=4)
        ax.add_patch(rect)
    ...
    ax.scatter(e_lon, e_lat, color='yellow', marker='*', s=220, edgecolors='black', zorder=6)
    
    plt.title(f"HELIOS V6 智慧導航監控 (含風場避障)")
    st.pyplot(fig)
